...
public interface CitaRepository extends JpaRepository<Cita, Integer>, JpaSpecificationExecutor<Cita> {
    ...
    @Query("SELECT c FROM citas c WHERE c.inmueble.usuario.idUsuario=:idUsuario AND c.estadoCita.idEstadoCita IN (1, 2, 3, 4, 5, 7)")
    List<Cita> findCitasCreadasParaElAgente(@Param("idUsuario") int idUsuario);
